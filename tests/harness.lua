...
-- Fires an event exactly as the WoW client would: to every frame registered
-- for it, not just the addon's main handler. Other code (the dev panel's
-- round-trip probe) registers its own listener and must receive it too.
function H.fireEvent(event, ...)
    return Mock.fireEvent(event, ...)
end
