...
Status: **reviewed and implemented.** Sections 1, 2, 4 and 5 are automated (175 tests,
...
## 6. Release build (`tests/release_check.lua`)

Answers "does the game still work with the developer-only files gone?" by building an actual
release and exercising the addon against it, with `GuildWordle_Dev.lua` genuinely **absent** — not
stubbed, not mocked away, just not there.

`tests/build_release.sh` applies the packager's own `toc_filter`/`lua_filter` sed expressions,
copied verbatim from BigWigsMods/packager, so this mirrors what CurseForge publishes rather than
approximating it. That distinction matters: the two filters behave differently, and assuming
otherwise is easy to get wrong.
- **`.toc`**: `#@debug@ … #@end-debug@` lines are **deleted**.
- **`.lua`**: `--@debug@` → `--[==[@debug@` and `--@end-debug@` → `--@end-debug@]==]`, wrapping the
  block in a **long comment** — the block is not removed, so it must not contain `]==]` or the
  comment closes early and the rest becomes live code.

Every other suite loads the dev file, so this is the only thing that can catch a release-only
break: a reference to a stripped function, a debug block that doesn't close cleanly, or a `.toc`
still naming a file that no longer ships.

- **REL-01**: The dev file is absent and its entry points (`GW.SetDevPanelShown`, `GW.DevActions`)
  don't exist.
- **REL-02**: The release `.toc` no longer names the stripped file, but *does* still list the real
  ones — i.e. the filter didn't overreach.
- **REL-03**: A full game plays through: guesses evaluate, the win registers, the streak advances.
- **REL-04**: Nicknames still set and sanitise.
- **REL-05**: Gossip (`RESULTS:`/`NICKS:`/`STREAKS:`) parses, stores, and renders without the panel
  reporting an internal error.
- **REL-06**: `/wordle dev` is not a command — it doesn't set `devMode` or announce anything, and
  falls through to the window toggle.
- **REL-07**: The Dev button stays hidden even if `devMode` is hand-set in SavedVariables.
- **REL-08**: Every ordinary slash command still works.
- **REL-09**: Debug blocks are wrapped as *well-formed* long comments — opening markers are all
  rewritten, every closing marker is followed by `]==]`, and open/close counts balance.

Verified by breaking each guard in turn: removing the `.toc` markers fails REL-02, removing the
Lua markers fails REL-06.

---

## 7. Cross-version compatibility (`tests/crossversion.lua`)

Runs the **actual released v1.0.4 code** (`tests/fixtures/v1.0.4_GuildWordle.lua`, taken from the
v1.0.4 tag) against the current build, in both directions. v1.0.4 is live on other people's
clients; if this version's new messages broke it, or its `RESULTS:` stopped parsing, guildmates
would silently stop seeing each other. Every other cross-version claim here was argued from reading
the code — the same reading that produced several shipped bugs — so this runs the old code instead.

- **XVER-01/02**: v1.0.4 survives this version's `NICKS:` and `STREAKS:` messages. Unknown message
  types fall through its if/elseif chain harmlessly, which is the property that makes adding new
  message types safe at all.
- **XVER-03**: v1.0.4 still reads this version's `RESULTS:` and stores it correctly — the reason
  that format was deliberately left byte-identical.
- **XVER-04**: Captures v1.0.4's own real broadcast output for the reverse direction (not a
  hand-written approximation of the old format).
- **XVER-05**: The current build parses those captured strings.
- **XVER-06**: An old client's result displays under its character name, since v1.0.4 never sends
  a nickname — must not render blank or nil.
- **XVER-07**: `RESULTS:` is byte-identical across versions, asserted directly against v1.0.4's
  emitted output and the current build's.

## 8. Real transport (manual, dev panel)

Everything else exercises the receive path with locally injected messages, which never touches
WoW's addon channel. **REAL round-trip test** in the dev panel is the only check of prefix
registration, `SendAddonMessage`, the server round trip, and `CHAT_MSG_ADDON` delivery.

It works solo because WoW delivers your own addon messages back to you — the same reason
`HandleAddonMessage` guards `SYNC_REQ` with `name ~= me`. It probes with an ordinary `SYNC_REQ`
rather than a fake result, since a fake would put a bogus player on real guildmates' leaderboards.
Requires being in a guild (any guild, including one you're alone in); it says so rather than
failing silently. Covered by DEV-15/15b/15c.
