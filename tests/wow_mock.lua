...
function MockState.reset()
    ...
    -- createdFrames is deliberately NOT cleared: WoW frames live for the
    -- whole session and are never destroyed, and addon code caches them in
    -- upvalues. Clearing the list would drop long-lived listeners (e.g. the
    -- dev panel's round-trip probe) out of fireEvent's dispatch while the
    -- code that created them still holds a reference -- an inconsistency the
    -- real client can't produce.
end
...
-- Dispatches to EVERY frame registered for the event, like the real client.
-- Driving only one frame would miss listeners other code sets up
-- independently -- e.g. the dev panel's round-trip probe watches
-- CHAT_MSG_ADDON on its own frame, alongside the addon's main handler.
function MockState.fireEvent(event, ...)
    for _, f in ipairs(MockState.createdFrames) do
        if f.IsEventRegistered and f:IsEventRegistered(event) then
