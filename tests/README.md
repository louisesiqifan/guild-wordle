...
| `build_release.sh`, `release_check.lua` | Builds a simulated release with the packager's own filters (dev panel stripped) and proves the addon still loads and plays without it. |
| `../GuildWordle_Dev.lua` | The in-game dev panel (not a test file, but what section 3's manual UAT is driven with). |

## Coverage

175 automated tests. Spec sections 1, 2, 4 and 5 are automated. **Section 3
...
Local `git` is unavailable in the usual dev environment here, so pushes go via
the GitHub API. Use the **Git Data API** (blobs → tree → commit → update ref)
to batch every changed file into **one** commit. The contents API
(`PUT /repos/.../contents/{path}`) creates a *separate commit per file*, which
is how this branch twice accumulated dozens of noise commits that had to be
squashed away afterwards.
