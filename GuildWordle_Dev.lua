...
-- Everything else in this panel exercises the RECEIVE path with locally
-- injected messages, which never touches WoW's actual addon channel. This is
-- the one action that proves the real transport works: prefix registration,
-- SendAddonMessage, the server round trip, and CHAT_MSG_ADDON delivery.
--
-- Works solo because WoW delivers your own addon messages back to you -- the
-- same reason HandleAddonMessage has to guard SYNC_REQ with `name ~= me`.
--
-- Uses SYNC_REQ deliberately: it's ordinary traffic every client already
-- sends at login, so a guildmate receiving it just re-broadcasts what they
-- know. Sending a fake RESULTS: instead would put a bogus player on real
-- guildmates' leaderboards.
local probeListener, probeState
function Actions.realRoundTrip()
    if not IsInGuild() then
        say("|cffff4444Not in a guild -- there's no channel to send on. "
            .. "Join or be invited to any guild (even one you're alone in) and retry.|r")
        return
    end

    if not probeListener then
        probeListener = CreateFrame("Frame")
        probeListener:RegisterEvent("CHAT_MSG_ADDON")
        probeListener:SetScript("OnEvent", function(_, _, prefix, text, channel, sender)
            if probeState and prefix == ADDON_PREFIX and text == "SYNC_REQ" then
                probeState.got = true
                probeState.sender = sender
                probeState.channel = channel
            end
        end)
    end

    probeState = {got = false}
    local me = UnitName("player")

    -- Sent directly rather than through GW's broadcasters so the Isolate
    -- toggle can't silently swallow it -- the whole point is a REAL send.
    if C_ChatInfo and C_ChatInfo.SendAddonMessage then
        C_ChatInfo.SendAddonMessage(ADDON_PREFIX, "SYNC_REQ", "GUILD")
    elseif SendAddonMessage then
        SendAddonMessage(ADDON_PREFIX, "SYNC_REQ", "GUILD")
    else
        say("|cffff4444No SendAddonMessage API available.|r")
        return
    end
    say("Sent a real SYNC_REQ over the guild addon channel; waiting for it to come back...")

    C_Timer.After(3, function()
        if probeState and probeState.got then
            say("|cff00ff00Round trip OK.|r The addon prefix is registered and messages "
                .. "genuinely send and arrive (echoed back from " .. tostring(probeState.sender)
                .. " on " .. tostring(probeState.channel) .. "). Guildmates running the addon "
                .. "will have replied with everything they know, so check the tabs.")
        else
            say("|cffff4444No echo after 3s.|r Either the addon-message prefix isn't "
                .. "registered, or the guild channel isn't delivering. This is the one "
                .. "failure mode local injection cannot detect -- worth investigating "
                .. "before release.")
        end
        probeState = nil
    end)
...
local SECTIONS = {
    {title = "Simulated guildmates (incoming messages)", buttons = {
        ...
        {text = "REAL round-trip test",       fn = Actions.realRoundTrip},
    }},
